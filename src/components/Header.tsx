    ...
    >
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        <Link href="/" className="flex flex-col leading-tight hover:opacity-80 transition-opacity">
          <span className="text-xs tracking-[0.25em] uppercase" style={{ color: "var(--stone)" }}>
            Queenstown
          </span>
      ...
      {open && (
        <nav
          className="md:hidden border-t px-6 py-4 flex flex-col gap-3 text-sm"
          style={{ borderColor: "var(--ash)", backgroundColor: "var(--ash)" }}
        >
          {rooms.map((room) => (
            <Link
              key={room.slug}
